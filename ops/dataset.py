...
class TSNDataSet(data.Dataset):
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            # print(self.num_segments,average_duration,offsets)
            return offsets + 1
    ...
    def get(self, record, indices):
        all_imgs = np.load(os.path.join(self.root_path,record.path+'.npy'))
        images = list()
        for seg_ind in indices:
            p = int(seg_ind)
            seg_imgs = [Image.open(BytesIO(all_imgs[p-1])).convert('RGB')]
